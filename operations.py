...
__all__ = ['ResBlock', 'ConvBlock', 'Skip','ConvNorm', 'OPS']
...
class ResBlock(nn.Module):
    ...
    conv => norm => activation
    use native Conv2d, not slimmable
    ResNet38 Blocks w/ 2 native Conv2d layer
    '''
    def __init__(self, C_in, C_out,  layer_id, expansion=1, kernel_size=3, stride=1, padding=None, dilation=1, groups=1, bias=False, num_bits_list=[0,]):
        ...
        self.conv1 = Conv2d(C_in, C_in, kernel_size=self.kernel_size, stride=1, 
                            padding=self.padding, dilation=1, groups=C_in, bias=bias, dws=True and DWS_CHWISE_QUANT)
        self.bn1 = BatchNorm2d(C_in, self.num_bits_list)

        self.conv2 = Conv2d(C_in, C_in, kernel_size=self.kernel_size, stride=self.stride, 
                            padding=self.padding, dilation=1, groups=C_in, bias=bias, dws=True and DWS_CHWISE_QUANT)
        self.bn2 = BatchNorm2d(C_in, self.num_bits_list)
        self.conv3 = Conv2d(C_in, C_out, kernel_size=1, stride=1, padding=0, dilation=1, groups=self.groups, bias=bias)
        self.bn3 = BatchNorm2d(C_out, self.num_bits_list)
        self.relu = nn.ReLU(inplace=True)
    ...
    def _flops(h, w, C_in, C_out, expansion=1, kernel_size=3, stride=1, padding=None, dilation=1, groups=1, bias=False):
        layer_id = 1
        layer = ResBlock(C_in, C_out, layer_id, expansion, kernel_size, stride, padding, dilation, groups, bias)
        flops, params = profile(layer, inputs=(torch.randn(1, C_in, h, w),), custom_ops=custom_ops)
        return flops
    ...
    def forward_flops(self, size):
        ...
        name = "ResBlock_H%d_W%d_Cin%d_Cout%d_exp_%dkernel%d_stride%d_group%d"%(h_in, w_in, c_in, c_out, self.expansion, self.kernel_size, self.stride, self.groups)
        if name in flops_lookup_table:
            flops = flops_lookup_table[name]
        else:
            print("not found in flops_lookup_table:", name)
            flops = ResBlock._flops(h_in, w_in, c_in, c_out, self.expansion, self.kernel_size, self.stride, self.padding, self.dilation, self.groups, self.bias)
            flops_lookup_table[name] = flops
            np.save(flops_file_name, flops_lookup_table)
...
OPS = {
    'r3_e1' : lambda C_in, C_out, layer_id, stride, num_bits_list: ResBlock(C_in, C_out, layer_id, expansion=1, kernel_size=3, stride=stride, groups=1, num_bits_list=num_bits_list),
    'r5_e1' : lambda C_in, C_out, layer_id, stride, num_bits_list: ResBlock(C_in, C_out, layer_id, expansion=1, kernel_size=5, stride=stride, groups=1, num_bits_list=num_bits_list),
